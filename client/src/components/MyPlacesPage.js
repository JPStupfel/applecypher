...
export default function MyPlacesPage() {
  ...
  function fetchPlaces(){fetch(`places?limit=${6}&offset=${offset}&search=${search}`).then(r=>r.json()).then(d=>{console.log(d);if (d.length){setPlaceList(d)} else {handleChangeOffset(-6);console.log('You have reached the last page!')}}).catch(e=>console.log(e))}
  function searchPlaces(){fetch(`places?limit=${6}&offset=${offset}&search=${search}`).then(r=>r.json()).then(d=>{console.log(d);setPlaceList(d)}).catch(e=>console.log(e));setOffset(0)}
  ...
  // create place cards
  const PlaceCards = placeList.map(e=><PlaceClientCard key={e.id} place={e} />)
  // for Search bar
                            ...
                            <div id="results_scroll" className="padding5" style={scrollStyle}>
                              <div id="roadtripRegionsContainer" className="margin-bottom- none" style={{"display": "block;"}}>
                                <form className="formCustom " name="regionSearch" onSubmit={(e)=>{e.preventDefault();}}>
                                  ...
                                  <input  onInput={handleChange} value={search} placeholder="Search Places..." type="text" name="region-gt2-textbasic" id="ac_regionname" style={{"width": "90%"}} className="ui-autocomplete-input" autoComplete="off"/>
                                </form>
                                <ul id="roadtripRegions" className=" grid-row grid-row--gutter grid-row--col-2 grid-row--card-min-200">
                                {placeList.length ? PlaceCards : "No Places Match That Description"}
                                </ul>
                                <div className="btn-group">
                                  <button className="btn btn-outline-inverse " onClick={()=>handleChangeOffset(-6)}>Previous</button>
                                  <button className="btn btn-outline-inverse">Showing items {offset + 1} to {offset + placeList.length}</button>
