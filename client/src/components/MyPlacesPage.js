...
export default function MyPlacesPage() {
  ...
  function fetchPlaces(){fetch(`places?limit=${6}&offset=${offset}&search=${search}`).then(r=>r.json()).then(d=>{if (d.length){setPlaceList(d)} else {handleChangeOffset(-6);console.log('You have reached the last page!')}}).catch(e=>console.log(e))}
  function searchPlaces(){fetch(`places?limit=${6}&offset=${offset}&search=${search}`).then(r=>r.json()).then(d=>{setPlaceList(d)}).catch(e=>console.log(e));setOffset(0)}
